...
if((isset($_SESSION['id']) || isset($_GET['user_identity'])) && !isset($_POST['updateProfileBtn'])){
	...
	//initializze an array to store any error message from the form
	$form_errors = array();

	//form validation
	$required_fields = array('email', 'username');

	//call the function to check empty field and merge the return data into the form_error array
	...
	if(empty($form_errors)){
		try{
			...
			$result = flashMessage("An error has occured in : " .$ex->getMessage());
		}
	}else{
